...
/* OLED 显存缓冲区：8 页，每页 128 列，对应 128x64 屏幕 */
uint8_t OLED_DisplayBuf[8][128];

void OLED_W_SCL(uint8_t BitValue)
{
	/* PE9 作为软件 I2C 的 SCL 时钟线 */
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_9, (GPIO_PinState)BitValue);
	volatile uint32_t delay = 40;
	while (delay--);
}

void OLED_W_SDA(uint8_t BitValue)
{
	/* PE8 作为软件 I2C 的 SDA 数据线 */
	HAL_GPIO_WritePin(GPIOE, GPIO_PIN_8, (GPIO_PinState)BitValue);
	volatile uint32_t delay = 40;
	while (delay--);
}
...
{
	/* OLED 使用 PE8/PE9 模拟 I2C，所以先打开 GPIOE 时钟 */
...
void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
	...
	/* 控制字节 0x40 表示后面发送的是 OLED 显示数据 */
	OLED_I2C_Start();
	OLED_I2C_SendByte(0x78);
	OLED_I2C_SendByte(0x40);
	for (i = 0; i < Count; i++) { OLED_I2C_SendByte(Data[i]); }
	OLED_I2C_Stop();
}
...
void OLED_Clear(void)
{
	...
	/* 清空显存缓冲区，真正清屏还需要调用 OLED_Update */
	for (j = 0; j < 8; j ++) { for (i = 0; i < 128; i ++) { OLED_DisplayBuf[j][i] = 0x00; } }
}
	...
	for (j = 0; j < (Height - 1) / 8 + 1; j ++)
	{
		for (i = 0; i < Width; i ++)
		{
			if (X + i >= 0 && X + i <= 127)
			{
				...
				{
					OLED_DisplayBuf[Page + j][X + i] &= ~(0xFF << Shift);
				}
				if (Page + j + 1 >= 0 && Page + j + 1 <= 7)
				{
					OLED_DisplayBuf[Page + j + 1][X + i] &= ~(0xFF >> (8 - Shift));
				}
			}
		}
	}
...
{
	int16_t Angle;
	Angle = (int16_t)(atan2(Y, X) / 3.14159265f * 180);
